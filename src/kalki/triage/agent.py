...
from kalki.schemas import AlertStatus, OCSFAlert, Severity, Verdict
from kalki.triage.llm_explainer import LLMExplainer
...
class TriageAgent:
    ...
    def triage(self, alert: OCSFAlert) -> TriageResult:
        ...
        if severity in {Severity.CRITICAL, Severity.HIGH, Severity.MEDIUM}:
            tech = f"{best_match['tactic']} / {best_match['technique']}" if best_match else "None"
            final_reason = self.explainer.explain(text, rule_reason, tech)
