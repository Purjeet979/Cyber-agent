"""
Kalki CLI — command-line interface for all platform operations.

Usage:
    kalki ingest <file>          Ingest logs from file
    kalki triage                 Run triage on pending alerts
    kalki investigate <alert_id> Investigate a specific alert
    kalki correlate              Correlate alerts into incidents
    kalki hunt [hypothesis]      Run threat hunts
    kalki dashboard              Launch web dashboard
    kalki demo                   Run full demo with sample data
    kalki status                 Show system status
"""
...
from kalki import __version__
from kalki.schemas import AlertStatus, Severity
...
def run_demo(data_file: str = None):
    ...
    from kalki.hunt import hunt
    from kalki.models import RuleEngine
    ...
    # Final Summary
    print(f"\n{'='*70}")
    print(f"  DEMO COMPLETE — Kalki Pipeline Summary")
    print(f"{'='*70}")
...
if HAS_CLICK:
    ...
    def cli():
        """Kalki — Autonomous SOC Analyst Platform"""
        pass
    ...
    def dashboard(host: str, port: int, debug: bool):
        ...
        from kalki.dashboard import create_dashboard_app
    ...
    @cli.command()
    @click.option("--data", default=None, help="Path to a specific sample data file to run")
    def demo(data: str):
    ...
    def cli():
        ...
        if not args or args[0] in ("-h", "--help"):
            ...
            return
        if args[0] == "--version":
            print(f"Kalki v{__version__}")
            return
        if args[0] == "demo":
            if len(args) > 1 and args[1] == "--data" and len(args) > 2:
                run_demo(args[2])
            else:
                run_demo()
        elif args[0] == "status":
